...
- [x] **T63 — 實作 Demo UI 與 artifact/evidence rendering**
  - **依賴**：T40、T41、T42、T62、T64
  - **需求**：CP-FR011-13、CP-FR010-01..07
  - **允許修改目錄**：`src/crypto_trust_agent/application/use_cases/demo_ui.py`、`src/crypto_trust_agent/presentation/api/demo_ui_api.py`、`src/crypto_trust_agent/presentation/api/demo_ui_composition.py`、`src/crypto_trust_agent/presentation/demo_ui/`、`tests/integration/core/`、`tests/e2e/`、`tests/architecture/test_clean_architecture.py`、`README.md`、`.kiro/specs/core-platform/tasks.md`
  - **目錄擴充原因**：Demo UI 需要 Application-owned orchestration/query/download boundary（組合既有 CreateTask、Preflight、StartFormalExecution use cases 與 ArtifactRepository Port），Presentation 不得直接存取 Repository；Core Owner 於第二次審核時核准。真實 local fake wiring 只允許位於明名 `demo_ui_composition.py` composition root；architecture test 精確限制該 root 只能匯入 Core local fakes/identity，README 則提供人工 Demo 的 uvicorn 啟動入口。
  ...
  - **完成證據**：2026-08-01 於 `feature/core-platform` 完成真實 local fake composition 與 HTTP 驗收；`python -B -m unittest tests.e2e.test_demo_ui -v`（9 passed）、`python -B -m unittest tests.integration.core.test_demo_ui_integration -v`（15 passed）、T64 publication bridge regression（18 passed）、完整 unit regression（134 passed）、contract regression（99 passed）、architecture regression（4 passed）、Core integration regression（59 passed）、完整 E2E regression（9 passed），`python -B -m compileall -q src tests`、`git diff --check` 與 module-level FastAPI uvicorn import smoke 均通過。E2E 從 FastAPI `TestClient`/ASGI HTTP 入口開始，提交前 artifact repository 與 Execution 為空，使用 shared `FakePlatformStore`、真實 CreateTask/Preflight/StartFormalExecution use cases、`FormalRunOrchestrator` 與 T64 publication bridge完成 `submit→preflight→start→orchestrate→publish`；測試檔無 `MagicMock`、預寫 `bundle()` 或直接 `.publish()`。首頁提供題目與一至兩個支援幣種表單，透過 verified local session/Bearer principal 執行；status 顯示 terminal/publication/partial/degradation/artifact 狀態，並提供 Final Report、Evidence List（含 lineage/assessment/claims）、Execution Log、Manifest 頁面及四份 minimum bundle 下載。Application-owned bounded registry對 Task/Execution/artifact 全路徑執行 principal ownership，artifact 下載驗證 allowlist、task/execution identity、schema、decoded-byte SHA-256/size及安全檔名；HTML renderer統一 escaping/sanitization，HTTP 加入 bounded body/query、identity override與duplicate credential拒絕、CSP/no-store/nosniff及固定安全錯誤。不健康 official dataset pre-flight 回安全 409，Execution/quota/artifact/step executor均維持空；publication只由 T64 orchestrator exactly once觸發。繁體中文化驗收確認首頁、正式分析狀態、最終分析報告、證據清單、執行紀錄、成果清冊、未驗證身分與安全錯誤頁皆使用 `<html lang="zh-Hant">`、UTF-8 與固定繁體中文 UI；預設問題為「請分析 BTC 目前的市場狀況，列出關鍵證據、信心與已知限制。」，中文問題可完成 HTTP submit 並產出七個 artifacts。`completed`／`success`／`complete` 等底層值只在 HTML 顯示層映射為「中文（原始代碼）」，JSON response、DTO、enum、schema、artifact type/format/filename 與 wire field均未修改；Demo-only fake report/evidence敘述改為繁中，ID、URL、hash、lineage、Decimal 與 provider semantics維持不變。`README.md` 記錄 PowerShell `PYTHONPATH`、`python -m uvicorn crypto_trust_agent.presentation.api.demo_ui_api:app --port 8000` 與 `http://127.0.0.1:8000/`。未連線 AWS/network，未 commit/push，T80保持未完成。
